...
=====================================================================
 ...
 修复记录:
   v46.1 - 修复 Gradio 6.x 兼容性问题
           ...
           - 新增模式选择（用户确认模式 / 自动模式）
   v50.0 - 连续上下文记忆
           - 使用 main.get_router() 全局单例
           - 所有 Web UI 会话共享同一个 RouterAgent 实例
           - 跨消息的对话上下文自然延续
=====================================================================
"""
...
# v50.0: 在模块加载时初始化全局单例 RouterAgent
# 确保 Web UI 整个生命周期内只初始化一次子 Agent
...
def respond(message: str, history: list) -> str:
    ...
    v50.0 更新：使用全局单例 RouterAgent，跨消息保持上下文记忆。
    ...
    try:
        ...
        # v50.0: 复用全局 router 实例，messages 自然延续
        from main import run_multi_agent
        result = run_multi_agent(message, router=_web_router)
