...
def git_init_repo(repo_path: str = "") -> str:
    ...
    # ⚠️ 高危操作：初始化 Git 仓库会创建 .git 目录，需要人工审批
    approval = request_human_approval(
        operation="git_init_repo",
        details=f"在 {target} 初始化 Git 仓库"
    )
    if not approval:
...
def git_create_branch(branch_name: str, repo_path: str = "") -> str:
    ...
    # ⚠️ 高危操作：创建/切换分支会修改仓库状态，需要人工审批
    approval = request_human_approval(
        operation="git_create_branch",
        details=f"分支: {branch_name} | 仓库: {target}"
    )
    if not approval:
        return "❌ 操作已取消：未获得人工审批。"
...
def git_commit(message: str, repo_path: str = "", auto_add: bool = True) -> str:
    ...
    # ⚠️ 高危操作：提交更改会修改 Git 仓库历史，需要人工审批
    approval = request_human_approval(
        operation="git_commit",
        details=f"提交信息: {message} | 仓库: {target}"
    )
    if not approval:
        return "❌ 操作已取消：未获得人工审批。"
...
def git_checkout(branch_name: str, repo_path: str = "") -> str:
    ...
    # ⚠️ 高危操作：切换分支可能丢失未提交的更改，需要人工审批
    approval = request_human_approval(
        operation="git_checkout",
        details=f"切换到分支: {branch_name} | 仓库: {target}"
    )
    if not approval:
        return "❌ 操作已取消：未获得人工审批。"
