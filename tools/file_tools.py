...
def read_file_by_lines(
    ...
    try:
        ...
        if end_line < total_lines:
            nav_parts.append(
                f"下一页: read_file_by_lines(start_line={end_line + 1}, end_line={min(end_line + 100, total_lines)})")
...
def write_local_file(file_path: str, content: str) -> str:
    ...
    approval = request_human_approval("write_local_file", {
        "file_path": abs_path,
        "content_preview": content_preview,
        "content_length": str(len(content)),
    })
    if approval == "REJECTED":
        return f"HITL 审批拒绝：已取消写入文件 {abs_path}"
    if approval.startswith("FEEDBACK:"):
        feedback = approval[9:]
        return f"HITL 审批反馈：{feedback}。文件写入已取消。"
...
def run_terminal_command(command: str) -> str:
    ...
    # HITL 人工审批
    approval = request_human_approval("run_terminal_command", {
        "command": command[:300],
    })
    if approval == "REJECTED":
        return f"HITL 审批拒绝：已取消执行命令: {command[:200]}"
    if approval.startswith("FEEDBACK:"):
        feedback = approval[9:]
        return f"HITL 审批反馈：{feedback}。命令执行已取消。"
...
def list_directory_tree(
    ...
    def _walk(dir_path, depth):
        if depth > max_depth:
            return
        try:
            entries = sorted(os.listdir(dir_path), key=lambda x: (
...
def check_python_syntax(file_path: str = "") -> str:
    ...
    try:
        ...
        result_parts.append(
            f"   统计: {len(source_code.split(chr(10)))} 行, {class_count} 个类, {func_count} 个函数")
            ...
            if isinstance(node, ast.Import):
                ...
                for alias in node.names:
                    ...
                    if _match_name(full_name):
                        result_parts.append(
                            f"  from {module} import {alias.name}  [行{node.lineno}]")
                        found = True
...
def grep_search(
    ...
    try:
        proc = subprocess.run(cmd, shell=True, capture_output=True,
                              timeout=SEARCH_TIMEOUT, text=True)
