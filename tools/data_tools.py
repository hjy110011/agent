        ...
        try:
            if data_format == "json":
                ...
                try:
                    ...
                    formatted = yaml.dump(parsed, default_flow_style=False, allow_unicode=True, indent=indent)
...
def code_format_tool(
    ...
    # ---------------------------------------------------------------
        ...
        if total_lines > 0:
            result_parts.append(f"  代码行:       {code_lines:>6}  ({code_lines/total_lines*100:.1f}%)")
            result_parts.append(f"  注释行:       {comment_lines:>6}  ({comment_lines/total_lines*100:.1f}%)")
            result_parts.append(f"  文档字符串行: {docstring_lines:>6}  ({docstring_lines/total_lines*100:.1f}%)")
            result_parts.append(f"  空行:         {blank_lines:>6}  ({blank_lines/total_lines*100:.1f}%)")
