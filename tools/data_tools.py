...
def file_backup_tool(
    ...
    if action == "backup":
        ...
        approval = request_human_approval(
            operation="文件恢复(file_backup_tool.restore)",
            details=f"将备份版本恢复到: {target_path}"
        )
        if not approval:
        ...
        try:
            ...
            try:
                ...
                if file_path:
                    ...
                    approval = request_human_approval(
                        operation="代码格式化(code_format_tool.format)",
                        details=f"将基础格式化后的代码写入: {abs_path}"
                    )
                    if not approval:
