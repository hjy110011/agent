...
## 任务分类规则
- **编程/文件操作任务**（如编写代码、修改文件、运行命令、代码审查等）
  → 转发给 CoderAgent
...
## 路由规则
1. 每次用户输入，你必须先评估任务类型
2. 调用 delegate_task(target_agent, task_context) 转发任务
3. 等待子 Agent 返回结果后，整理并汇报给用户
4. 如果任务涉及多个领域，可以分步转发
...
"""
    ...
    def __init__(self, coder_agent=None, dba_agent=None):
        """
        初始化 RouterAgent。
    ...
    def _create_delegate_tool(self):
        ...
        def delegate_task(target_agent: str, task_context: str) -> str:
            ...
            Args:
                target_agent: 目标子 Agent 名称（"CoderAgent" 或 "DBAAgent"）
    ...
    def _handle_delegate(self, target_agent: str, task_context: str) -> str:
        ...
        if target_agent == "CoderAgent" and self.coder_agent:
