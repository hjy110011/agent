"""
=====================================================================
 🚀 多智能体路由入口 (v37.0 多智能体路由版)
 
 架构说明：
   - RouterAgent：路由调度核心，评估用户任务并转发给子 Agent
   - CoderAgent：编程/文件操作子智能体
   - DBAAgent：数据库/数据格式子智能体
   
 每个子 Agent 维护自己独立且绝对静态的 messages 上下文列表，
 以最大化 Cache Hit。
=====================================================================
"""

import os
import sys
import time
import logging

# 将项目根目录加入 sys.path
_project_root = os.path.abspath(os.path.dirname(__file__))
if _project_root not in sys.path:
    sys.path.insert(0, _project_root)

from core.config import SAFE_BASE_DIR, DEFAULT_MAX_STEPS, logger
from core.utils import _TimeoutError, _input_with_timeout
from agents.router_agent import RouterAgent
from agents.coder_agent import CoderAgent
from agents.dba_agent import DBAAgent


# =====================================================================
# 🚀 多智能体路由入口
# =====================================================================
def run_multi_agent(user_task: str, max_steps: int = DEFAULT_MAX_STEPS) -> None:
    """
    多智能体路由入口。
    
    用户输入将首先交给 RouterAgent 评估，RouterAgent 决定转发给
    对应的子 Agent 执行，子 Agent 处理完毕后将结果封装返回给
    RouterAgent 最终汇报给用户。
    
    Args:
        user_task: 用户任务描述
        max_steps: 最大循环步数
    """
    print("\n" + "=" * 60)
    print("[🤖 多智能体路由系统 (v37.0) 已启动]")
    print(f"📋 任务: {user_task[:100]}{'...' if len(user_task) > 100 else ''}")
    print("=" * 60)
    
    # 初始化子 Agent（每个子 Agent 维护独立的 messages 上下文）
    print("\n[系统] 初始化子智能体...")
    coder_agent = CoderAgent()
    dba_agent = DBAAgent()
    print("[系统] CoderAgent ✅ 已就绪")
    print("[系统] DBAAgent ✅ 已就绪")
    
    # 初始化 RouterAgent（传入子 Agent 引用）
    router = RouterAgent(coder_agent=coder_agent, dba_agent=dba_agent)
    print("[系统] RouterAgent ✅ 已就绪")
    print("=" * 60)
    
    # 将任务交给 RouterAgent 处理
    result = router.run(user_task, max_steps)
    
    # 最终输出
    if result:
        print("\n" + "=" * 60)
        print("[📋 最终处理结果]")
        print(result)
        print("=" * 60)


# =====================================================================
# 🚀 主程序入口
# =====================================================================
if __name__ == "__main__":
    print("=" * 60)
    print("🚀 企业级多智能体路由系统 v37.0")
    print(f"🔒 安全目录: {SAFE_BASE_DIR}")
    print(f"📋 架构: RouterAgent → [CoderAgent | DBAAgent]")
    print("=" * 60)
    
    if len(sys.argv) > 1:
        run_multi_agent(" ".join(sys.argv[1:]))
    else:
        while True:
            try:
                print("\n" + "-" * 50)
                print("  [多行输入模式]")
                print("  - 输入单独一行 EOF 结束")
                print("  - 连续两次回车(空行)自动提交")
                print("  - 输入 q 单独一行退出程序")
                print("-" * 50)
                
                lines = []
                empty_line_count = 0
                while True:
                    try:
                        line = _input_with_timeout(prompt="", timeout=300)
                    except _TimeoutError:
                        print("  [输入超时，使用默认测试任务]")
                        task = "请帮我检查当前目录下的 Python 文件"
                        break
                    if line.strip().lower() in ['q', 'quit', 'exit'] and not lines:
                        print("  Bye!")
                        raise SystemExit(0)
                    if line.strip() == 'EOF':
                        break
                    if line == '':
                        empty_line_count += 1
                        if empty_line_count >= 2:
                            while lines and lines[-1] == '':
                                lines.pop()
                            break
                    else:
                        empty_line_count = 0
                    lines.append(line)
                
                task = "\n".join(lines).strip()
                if not task:
                    print("  [输入为空，请重新输入]")
                    continue
                run_multi_agent(task)
            except SystemExit:
                break
            except KeyboardInterrupt:
                print("\n\n  Bye!")
                break
            except Exception as e:
                logger.error(f"系统异常: {e}")
